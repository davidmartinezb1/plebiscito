<?php
$xml2= new SimpleXMLElement($name_file_nac, null, true);
$key=0;
$a=0;
...
//for($i=0;$i<33;$i++){
	$nombre='colombia';
	$data_nac[$nombre]=array();
	$data_nac[$nombre]['si']=$si; // Votos por el SI
	$data_nac[$nombre]['no']=$no; // Votos por el No
	$data_nac[$nombre]['data'][1]=$n_boletin;// numero de boletin
	$data_nac[$nombre]['data'][2]=$n_hora;// Hora de actualizacion
	$data_nac[$nombre]['data'][3]=$n_fecha;// Fecha de actualizacion
	$data_nac[$nombre]['data'][4]=$n_mesas;// Mesas informadas de las Instalas
	$data_nac[$nombre]['data'][5]=$n_votos;// Votos contados del potencial de sufragantes
	$data_nac[$nombre]['data'][6]=$n_votonulos;// Votos nulos
	$data_nac[$nombre]['data'][7]=$n_nomarcados;// Votos no marcados*/
//}
/*Fin ciclo*/
...
$json_string=json_encode($data_nac);
$file = 'js-vector/data-nac.json';
